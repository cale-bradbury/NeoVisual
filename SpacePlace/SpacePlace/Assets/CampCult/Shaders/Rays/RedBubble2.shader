Shader "Camp Cult/Rays/RedBubble2" {
Properties {
...
float2 map( in float3 pos, float t )
{
    ...
    //pos.xy = 
    float3 p = pos;
   	//p.zy = opKale(p.zy, pi/3.0,pi*1.3333, t*pi/6.+pi*.25);
   // p.x+=sin(p.y*2.+t*pi+length(pos.xy))*.5;
    //p.y+=cos(cos(pos.x*1.)*3.+t*pi)*.14;
    float s = (sdSphere(p,float3(1.3,1.3,1.3)));
   	float amp = 2.5;
	for(float i = 0.; i<1.;i+=.1){
	float ph = fmod(t+i,1.0);
	float3 ad = float3(cos(i*pi*2.)*ph*amp,
						sin(i*pi*2.)*ph*amp,
						0.);
						
		s = smin(s,sdSphere(p+ad, (1.0-ph)*.7),5.);
	}
