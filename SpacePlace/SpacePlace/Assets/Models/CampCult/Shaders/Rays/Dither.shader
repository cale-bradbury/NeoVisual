...
#include "ray.cginc"
	...
	float2 map(in float3 pos, float t)
	{
		...
		//pMod1(pos.x, 1.);
		float d = 1.0-floor(pos.y);
		float s = 1000.;
		float z = mod(t, 1.)*.1;
		/*for (float i = z - .5; i < 1. + z; i += .1) {
			s = fOpUnionSmooth(s, fSphere(pos + float3(0., ulerp(-5., 5., i), 0.), ulerp(1., 0., i)), .5);
		}*/
		s = (sin(pos.z+t)+sin(pos.x)+sin(pos.y-t)+pos.y*.1);
		s = fOpDifferenceSmooth(s, fSphere(pos, 40.), 2.5);
		res.x = s;
		return res;
	}
