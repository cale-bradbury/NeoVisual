...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ColorCorrectionCurves : PostEffectsBase
	{
        ...
        private Material CampMaterial;
        private Material CampDepthMaterial;
        private Material selectiveCampMaterial;
        ...
        public bool  selectiveCamp = false;
        ...
        public override bool CheckResources ()
		{
            ...
            CampMaterial = CheckShaderAndCreateMaterial (simpleColorCorrectionCurvesShader, CampMaterial);
            CampDepthMaterial = CheckShaderAndCreateMaterial (colorCorrectionCurvesShader, CampDepthMaterial);
            selectiveCampMaterial = CheckShaderAndCreateMaterial (colorCorrectionSelectiveShader, selectiveCampMaterial);
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination)
		{
            ...
            if (useDepthCorrection)
			{
                CampDepthMaterial.SetTexture ("_RgbTex", rgbChannelTex);
                CampDepthMaterial.SetTexture ("_ZCurve", zCurveTex);
                CampDepthMaterial.SetTexture ("_RgbDepthTex", rgbDepthChannelTex);
                CampDepthMaterial.SetFloat ("_Saturation", saturation);

                Graphics.Blit (source, renderTarget2Use, CampDepthMaterial);
            }
            else
			{
                CampMaterial.SetTexture ("_RgbTex", rgbChannelTex);
                CampMaterial.SetFloat ("_Saturation", saturation);

                Graphics.Blit (source, renderTarget2Use, CampMaterial);
            }

            if (selectiveCamp)
			{
                selectiveCampMaterial.SetColor ("selColor", selectiveFromColor);
                selectiveCampMaterial.SetColor ("targetColor", selectiveToColor);
                Graphics.Blit (renderTarget2Use, destination, selectiveCampMaterial);
