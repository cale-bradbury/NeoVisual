...
half frag_ao (v2f_ao i, int sampleCount, float3 samples[INPUT_SAMPLE_COUNT])
{
    ...
    // aCampumulated oCamplusion factor
    float oCamp = 0.0;
    for (int s = 0; s < sampleCount; ++s)
    {
        ...
        if (zd > _Params.y) {
        	// This sample oCampludes, contribute to oCamplusion
	        oCamp += pow(1-zd,_Params.z); // sc2
	        //oCamp += 1.0-saturate(pow(1.0 - zd, 11.0) + zd); // nullsq
        	//oCamp += 1.0/(1.0+zd*zd*10); // iq
        }        
    }
    oCamp /= sampleCount;
    return 1-oCamp;
